...
- **経費申請・ワークフロー機能**
  - **申請タイプ**: 「立替払い（領収書）」 / 「事前出金（見積書・請求書）」 【完了済み】
  - **購入方法**: 「Web購入（画像/PDF提出必須）」 / 「実店舗購入（紙の領収書を手渡し）」 【完了済み】
  - **定期支払い**: 月次・年次の定期テンプレート登録と当月/当年データの自動生成・督促 【完了済み】
  - **ステータス管理**: `PENDING_APPROVAL` (承認待ち) ➔ `APPROVED` (承認済/証憑待ち) ➔ `WAITING_CONFIRMATION` (最終確認待ち) ➔ `COMPLETED` (完了) 【完了済み】
...
- **管理・補助機能**
  ...
  - **監査ログ (AuditLogs)**: 申請変更・ステータス更新・画像差し替えの操作履歴の保持・閲覧 【完了済み】
  - **年度リセット**: 単年度予算制（4/1〜3/31）に応じたデータ切り替え 【完了済み】
